...
> 本文件是 seedance-prompt skill 仓库的交接入口。新会话开始：先读 `README-安装说明.md`、`references/paths.md`、`TODO.md`，再按 `SKILL.md` 工作流执行。

## 项目
seedance-prompt skill（生成 Seedance 口播提示词）+ 六维评分工具，git 仓库跨机/团队使用。
仓库根：`C:\Users\<用户名>\.zcode\skills\seedance-prompt\`（git init，main；规则截至第21条，2026-09-08）。

## 远程与同步
- **双远程**：GitHub 私有 `heronbo111/seedance-prompt`（作者主库）；Gitee 公开 `HeronBo/seedance-prompt`（国内直连；公开版已脱敏，真实数据只存各机本机样本库）。
