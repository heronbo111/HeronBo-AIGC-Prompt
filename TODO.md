...
- **双远程已建并推送完成**：GitHub **私有** `heronbo111/seedance-prompt`（作者主库）；Gitee **公开** `HeronBo/seedance-prompt`（国内直连使用；该账号"私有"选项禁用=仅支持公开仓库，若日后解锁可在设置转私有）。Gitee 推送凭据=私人令牌，已存入本机 GCM（`credential.helper=manager`，repo 级已配）。
...
3. **同步节奏（2026-09-08 用户定）**：每周一 09:00 自动推送 cron 已建（automation-45fe6fb0）；平时改动只提交不推送，有急更新需提前同步时可在任意会话手动 push。
