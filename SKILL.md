...
- 每类的模板骨架、示例与禁令见 `references/prompt-templates.md`；模型的输出/输入上限见 `references/platforms.md` 的「模型与限额」表。
...
- **先按「四大类判定表」定路线**（图生/文生/参考生/参考替换），再取对应模板骨架。
...
- **按「用户的需求与台词文案」判断，不是按素材判断**：用户说"把视频里的人换成另一个人""复刻这段动作"→ 替换类/参考视频类；用户只给文案＋图/音频、没有要复刻的视频 → 生成类。**素材只是执行条件，不决定分类。**
  - **但「不决定分类」≠「不用看素材」**——两句话说的是两件事：**分类由需求定（走哪条路线、取哪套模板骨架），写什么由素材定**。素材必须逐项看清并归纳出各自职责（哪份管形象/台词/音色/动作），这份总结直接决定提示词正文能写什么；**不看素材既归不了类，也写不出提示词**。识别手法见 rules.md 第11条，职责写法见第30条。
...
**提示词不是固定模板**：先定分类（图生/文生/参考生/参考替换；旧称生成类 A/B、参考视频类 C、替换类 L1–L4），再取对应模板骨架（`references/prompt-templates.md`）；分节名可合并，硬项一条不省。
...
2. **扫项目出提示词**：读 `骨架.json`（素材清单带 `role`）+ `文案/`（用户给的需求）+ `素材/`，
   按规则出提示词 → 写回 `骨架.json` 的 `prompts` 与 `文案/提示词.txt`。
