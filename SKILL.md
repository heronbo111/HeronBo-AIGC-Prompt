...
- 生成结果的评价由用户用**评价工具（窗口版，交付后由 agent 弹出）**完成，废片归档同样归用户；本 skill 只回收这些反馈来更新规则。
...
4. 登记路径：样本库根 = 项目目录的上一级（评价工具连接这一级），写入 `paths.local.md`。
...
4. **硬性要求：交付完必须把评价工具弹出来**（2026-09-14 用户要求）——这是交付动作的一部分，不是可选项；不许只留一句「记得打分」就算完。
...
- 上游 `version` 变大：更新后自检 `python tools\首次配置.py`（无参）、`python tools\评价回收.py`、评价工具能弹出（`tools\score_gui.cmd`）；`rules.local.md` 与上游新规则冲突时**以本机实测为准**并提示用户。
