...
- **流程指挥台**（右上「？流程指挥台」）：整条流程一张板子 —— `tools/workbench/flow.html` 是**独立 HTML 页**，
  用 iframe 嵌进工作台（不是弹窗），可单独打开看。五步各一张卡，写明「谁在做 / 现在什么状态 / 这一步怎么做」，
  卡上的按钮**当场就能执行**（选素材 · 选文件夹 · 建框架归类 · 出提示词 · 复制提示词+清单 · 收成片 · 收废片 ·
  去④栏打分 · 提交反馈）；出提示词时进度条与日志就显示在那张卡里。板上只画界面，动作发回 `app.js` 执行
  （一条命令通道，不留两份实现）；主题跟着工作台走。
