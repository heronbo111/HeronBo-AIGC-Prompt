...
| 成片打分（命令行） | `tools\评分.py`（`--list` 清单 / 问答式打分 / `--show` 看历史）；网页版备选 `评价工具.html` |
...
**判断分流（2026-09-13 用户要求）**：每收到一条新的提示词需求，**先判断是不是新项目**——
- **是新项目** → 走本节：先建骨架（`文案/ 素材/ 成片/ 废片/ 评价/ 备注/`，交付提示词时再加 `即梦上传/`），再动手写提示词；
...
4. **交付后立刻给评分入口，并且要真的做到**（2026-09-14 用户批评：上次没做）——两条路，**首选命令行版**：
   - **命令行（首选，不需要浏览器）**：`python tools\评分.py`（问答式：列样本 → 选成片 → 六维+违禁项+结论+备注 → 自动写 `<样本>/评价/日期-<成片>.json`）；用户也可以直接双击 `tools\score.cmd`。
   - **网页版（备选）**：`python -m http.server 8787 --directory "<本包 tools 路径>"` → 打开 `http://localhost:8787/评价工具.html`。
   - 两条路写出的 json **完全等价**，`tools\评价回收.py` 都能回收。agent 要在交付说明里给出**可复制的命令**，并在用户说"成片出来了"时主动跑 `--list` 看有没有待评。
   固定话术：「成片出来后，用评分程序打一下分（六维+违禁项+结论），我会根据你的反馈更新规则，并问你要不要优化 skill」
