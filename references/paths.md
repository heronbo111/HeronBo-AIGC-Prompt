...
拿到答案后：① 建项目骨架（`文案/素材/成片/废片/评价/备注`，交付提示词时再加 `即梦上传/`）；② 把用户提供的素材/文案自动归类落位；③ 按 `references/platforms.md` 处理平台 CLI（**有才装、没有不装、不检测账号**）；④ 写入 `paths.local.md`。
