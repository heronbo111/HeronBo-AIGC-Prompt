...
1. 把本 skill 仓库 clone 到 `~/.zcode/skills/`（ZCode 用户级；Codex 则是 `~/.codex/skills/`）：
   `git clone https://github.com/heronbo111/seedance-prompt.git C:\Users\你的用户名\.zcode\skills\seedance-prompt`（国内直连可改 clone `https://gitee.com/HeronBo/seedance-prompt.git`）
