...
def check(path, text, pdir=""):
    ...
    # ⑤.5 正文必须带 @（2026-09-21 加；用户实测："工作台生成的提示词没有 @ 符号了"）
    #   有「上传：」行说明这条用了素材 → 正文里就该有 @图片N/@视频N/@音频N；
    #   一个都没有，多半是把 @图片1 写成了「图片1」——平台认不出引用，整条会跑偏。
    if uploads:
        _body = text[text.find("上传："):]
        _plain = [m.group(0) for m in PLAIN_RE.finditer(_body)]
        if not re.search(r"@(?:图片|视频|音频)\d+", _body):
            say("FAIL", "正文里的 @ 引用", "有「上传：」行（用了素材），但正文里一个 @图片/@视频/@音频 都没有"
                "——引用素材要写成 @图片1、@视频1 这种")
        elif _plain:
            say("FAIL", "正文里的 @ 引用", "这 %d 处提到了素材却没带 @：%s ——**每一处提到素材都要带 @**"
                "（2026-09-21 用户裁定：一段里提到 4 次就该有 4 个 @）"
                % (len(_plain), "、".join(dict.fromkeys(_plain))[:60]))
        else:
            say("PASS", "正文里的 @ 引用")
    # ⑤.5 正文必须带 @（2026-09-21 加；用户实测：工作台生成的提示词没有 @ 符号了）
    #   有「上传：」行说明这条用了素材 → 正文里就该有 @图片N/@视频N/@音频N；一个都没有，
    #   多半是把 @图片1 写成了「图片1」——平台认不出引用，整条会跑偏。
    if uploads and not re.search(r"@(?:图片|视频|音频)\d+", text[text.find("上传："):]):
        say("FAIL", "正文里的 @ 引用", "有「上传：」行（用了素材），但正文里一个 @图片/@视频/@音频 都没有"
            "——引用素材要写成 @图片1、@视频1 这种（每条素材至少一次），写成「图片1」平台认不出来")
    elif uploads:
        say("PASS", "正文里的 @ 引用")
