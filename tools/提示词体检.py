...
# 裸写的素材编号（后面不是数字/下划线/点/短横 → 排除 图片1_四视图3.png 这类文件名）
PLAIN_RE = re.compile(r"(?<!@)(图片|视频|音频)\d(?![0-9_.\-])")
...
def check(path, text, pdir=""):
    ...
    # ⑤.5 正文的 @ 两条硬要求（rules 58；2026-09-22 合并了两个功能重复的老块，并加密度检查）：
    #   · 每次提到素材都必须带 @（裸写「图片1」平台认不出 → 整条跑偏）；
    #   · 但别反复点名同一件：密度对标段落式样本（全篇约 5 个 @）——
    #     2026-09-22 用户裁定"工作台产出的提示词 @ 过多，不符合 skill 里的要求"。
    if uploads:
        _body = text[text.find("上传："):]
        _plain = [m.group(0) for m in PLAIN_RE.finditer(_body)]
        _ats = re.findall(r"@(?:图片|视频|音频)\d+", _body)
        if not _ats:
            say("FAIL", "正文里的 @ 引用", "有「上传：」行（用了素材），但正文里一个 @图片/@视频/@音频 都没有"
                "——引用素材要写成 @图片1、@视频1 这种")
        elif _plain:
            say("FAIL", "正文里的 @ 引用", "这 %d 处提到了素材却没带 @：%s ——**每一处提到素材都要带 @**"
                "（rules 58；2026-09-21 用户实测：一段里提到 4 次就该有 4 个 @）"
                % (len(_plain), "、".join(dict.fromkeys(_plain))[:60]))
        else:
            say("PASS", "正文里的 @ 引用")
        # 密度：同一素材被点名太多 = 拿 @ 当标点。阈值留宽（替换类毫秒逐拍天然多 @），
        # 只在明显堆砌时报"注意"，不 FAIL、不拦交付。
        _cnt = {}
        for _a in _ats:
            _cnt[_a] = _cnt.get(_a, 0) + 1
        _top = max(_cnt.values()) if _cnt else 0
        if _top >= 6 or len(_ats) >= 15:
            say("WARN", "正文里的 @ 密度",
                "「%s」出现 %d 次（全篇 %d 个 @）——点名偏多：对照段落式样本（全篇约 5 个 @），"
                "把重复提及合并成一处；合并后每次提及仍要带 @（rules 58，2026-09-22 加）"
                % (max(_cnt, key=_cnt.get), _top, len(_ats)))
